...
def get_bokeh_all_graph():

    xget = get_all_timestamps()
    yget = get_all_temperatures()
    zget = get_all_outside_temps()
    wget = get_all_feels_like_temps()
    ...
    for i in zget:
        outside.append(float(i[0]))

    for i in yget:
        alltemp.append(float(i[0]))
        
    for i in wget:
        feels.append(float(i[0]))

    output_notebook()
    output_file('templates/bokehall.html')

    p = figure(title='Temperatures', x_axis_type='datetime', y_axis_label='y')
